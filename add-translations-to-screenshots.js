 ...
 */
async function addTranslationsToScreenshot(jsonPath, screenshotPath, outputPath) {
  try {
    // Read JSON data
    ...
    if (!entry) {
      entry = jsonData.find(item => {
        if (item.screenshotPath) {
          ...
          const screenshotWithoutTimestamp = screenshotName.replace(/_\d{4}_query-/, '_query-');
    ...
    const isAlreadyAnnotated = fs.existsSync(screenshotPath) && imgHeight > 1200; // reasonable threshold
...
async function processDateFolder(dateFolder) {
  ...
  const screenshotDir = path.join(__dirname, 'results', 'screenshots', dateFolder);
  ...
  for (const jsonFile of jsonFiles) {
    ...
    for (const subdir of screenshotSubdirs) {
      ...
      if (fs.statSync(subdirPath).isDirectory()) {
        const screenshots = fs.readdirSync(subdirPath).filter(f => f.endsWith('.png'));
        ...
        for (const screenshot of screenshots) {
          ...
          await addTranslationsToScreenshot(jsonPath, screenshotPath, outputPath);
        }
      }
    }
  }
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log('Example: node add-translations-to-screenshots.js 2026-04-09_PREPROD');
    console.log('\nAvailable date folders:');
    const screenshotDirs = fs.readdirSync(path.join(__dirname, 'results', 'screenshots'));
  ...
  console.log('\n✓ Done! Annotated screenshots saved in results/screenshots/' + dateFolder);
}
