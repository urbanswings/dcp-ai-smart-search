...
const FACETS_MASTER_DATA_PATH = path.resolve(
  __dirname,
  "tests/data/facets-master-data.json",
);
...
async function syncFacets(emhApiResponse, marketLanguage) {
  const emhFacets =
    emhApiResponse &&
    emhApiResponse.data &&
    emhApiResponse.data.search &&
    emhApiResponse.data.search.facets;
  ...
  try {
    ...
    if (err.code === "ENOENT") {
      ...
      console.error(
        `Warning: Could not parse ${FACETS_MASTER_DATA_PATH}: ${err.message}. Starting fresh.`,
      );
    }
  }
  const addedKeys = [];
  ...
  for (const [facetKey, rawFacet] of Object.entries(emhFacets)) {
    ...
    if (Array.isArray(sanitizedFacet.values)) {
      ...
      const existingIds = new Set(
        masterFacet.values.map(getFacetValueIdentifier),
      );
...
async function fetchMarket(market) {
  ...
  if (response.status !== 200) {
    throw new Error(
      `HTTP ${response.status}: ${typeof data === "string" ? data.substring(0, 200) : "non-JSON response"}`,
    );
  }
  ...
  if (typeof data === "string") {
    ...
    try {
      ...
      throw new Error(
        `JSON parse error: ${err.message}. Response preview: ${preview}...`,
      );
    }
  }
...
async function main() {
  ...
  console.log(
    `Markets: ${MARKETS.map((m) => `${m.country}-${m.product}`).join(", ")}\n`,
  );
  ...
  for (const market of MARKETS) {
    ...
    try {
      ...
      const { addedKeys, addedValues } = await syncFacets(
        data,
        market.language,
      );
      ...
      if (addedKeys.length > 0)
        console.log(`    new keys: ${addedKeys.join(", ")}`);
      succeeded++;
    } catch (err) {
  ...
  if (failed === 0) {
    console.log(
      "tests/data/facets-master-data.json updated with all market deltas.",
    );
  }
